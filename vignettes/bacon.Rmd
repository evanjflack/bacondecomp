...
\item{}{When including controls there is an additional source of idnetifying variation that comes from within treatent timing group. It is noted as $\hat{\beta^p_w}$}. $\Omega$ is the weight given to this estimate in the two-way fixed effects estimate. 
\item{}{The 2x2s are no longer decomposed into "early vs late" and "late vs early" estimates. For each treatment group dyad, there is only one estimate, $\hat{\beta}^d_{b, k, l}$ which gets weight $s_{k, l}.
