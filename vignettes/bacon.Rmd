...
## Description
`bacon()` is a function that decomposes two-way fixed effects models into all 2x2 estimates and their weights following Goodman-Bacon (2019). It can perform the decompostion with and without time-varying covariates.
...
Here, we perform the decompositin without time-varying controls, and make sure that the weighted average of the decomposition equals the two-way fixed effects estimate.
...
bacon_coef_cont <- Sigma*beta_hat_w + (1 - Sigma)*beta_hat_b
